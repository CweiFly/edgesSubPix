...
#pragma omp parallel for
    ...
    {
        Mat blur;
        GaussianBlur(gray, blur, Size(7, 7), alpha, alpha);
        ...
        // CHAIN_APPROX_NONE
        // CHAIN_APPROX_SIMPLE
        findContours(edge, contoursInPixel, hierarchy, RETR_TREE, CHAIN_APPROX_SIMPLE);
        int size = contoursInPixel.size();

        // for (int i = 0; i < size; i++)
        // {
        //     approxPolyDP(contoursInPixel[i], contoursInPixel[i], 2, true);
        // }
